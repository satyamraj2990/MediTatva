...
export const MediCallSarthi = ({ onClose }: MediCallSarthiProps) => {
  ...
  const initiateCall = async () => {
    ...
    try {
      ...
      if (errorData?.errorType === 'unverified_number') {
      ...
      } else if (errorData?.errorType === 'twilio_config_invalid') {
        const details = Array.isArray(errorData?.issues) ? errorData.issues.join(' | ') : 'Check Twilio env values.';
        toast.error('Twilio Config Missing/Invalid', {
          description: details,
          duration: 9000
        });
        console.error('\n🔧 TWILIO CONFIG INVALID');
        console.error('Set valid values in meditatva-backend/.env for:');
        console.error('TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER');
      } else if (errorData?.errorType === 'twilio_auth_failed') {
        toast.error('Twilio Authentication Failed', {
          description: 'Account SID/Auth Token is incorrect. Update .env and restart backend.',
          duration: 9000
        });
        console.error('\n🔐 TWILIO AUTH FAILED');
        console.error(errorData?.help || 'Update TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN in meditatva-backend/.env');
        ...
        console.error('Backend URL should match VITE_API_URL (or use /api with Vite proxy).');
        console.error('Check if backend is running and reachable from this frontend environment.');
      } else {
