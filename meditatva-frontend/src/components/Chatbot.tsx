...
try {
  ...
  const isPlaceholderKey =
    !apiKey ||
    /your[_-]?gemini[_-]?api[_-]?key/i.test(apiKey) ||
...
export const Chatbot = ({ onClose }: ChatbotProps = {}) => {
  ...
  const getBackendAssistantResponse = async (userInput: string): Promise<string> => {
    const conversationHistory = messages.slice(-6).map((m) => ({
      role: m.isBot ? "assistant" : "user",
      text: m.text,
    }));

    const response = await apiClient.post('/voice-assistant/query', {
      query: userInput,
      conversationHistory,
    });

    const backendText = response?.data?.response;
    if (!backendText || typeof backendText !== 'string') {
      throw new Error('Invalid backend assistant response');
    }
  ...
  const initializeChatSession = async () => {
    try {
      if (!genAI) {
        console.warn("Frontend Gemini key missing/invalid. Chatbot will use backend AI fallback.");
        setMessages([{
          text: "👋 **Hello! I'm MediTatva, your AI Health Assistant.**\n\nHow are you feeling today? You can tell me your symptoms, mood, stress, or medicine questions, and I will help you with practical guidance. 😊",
          isBot: true,
          timestamp: new Date(),
        }]);
        return;
      ...
      const chat = model.startChat({
        ...
        history: [
          {
            role: "user",
            parts: [{ text: SYSTEM_PROMPT + '\n\nSPECIAL RULE: If user says they are depressed/sad/anxious, ALWAYS provide: probable signs/symptoms, simple remedies, daily exercises/breathing steps, and when to seek professional help.' }],
          },
          {
            role: "model",
            parts: [{ text: "Ready. I will answer user health queries with practical guidance, symptoms, remedies, exercises, and safety advice in the user's language." }],
          },
        ],
      });
      ...
      // Start with a clean chat window so users can ask directly.
      setMessages([]);
    } catch (error) {
  ...
  const handleSend = async () => {
    ...
    setInputValue("");
    setIsTyping(true);
    const backendTriedFirst = !chatSession || !genAI;

    try {
      if (!chatSession || !genAI) {
        const backendResponse = await getBackendAssistantResponse(currentInput);
      ...
      // Retry with backend assistant if primary Gemini path fails.
      if (!backendTriedFirst) {
        try {
          const backendResponse = await getBackendAssistantResponse(currentInput);
          const fallbackResponse: Message = {
            text: backendResponse,
          ...
          console.error("❌ Backend assistant fallback failed:", backendError);
        }
      }
      ...
      if (errorMsg.includes("quota") || errorMsg.includes("429") || errorMsg.includes("RESOURCE_EXHAUSTED")) {
        ...
        errorText = currentLanguage === 'hi'
          ? "मैं आपकी बात समझ रहा हूं। अगर आप उदास या तनाव में हैं, तो अभी 3 चीजें करें: 1) 5 मिनट गहरी सांस लें, 2) पानी पिएं और किसी भरोसेमंद व्यक्ति से बात करें, 3) अगर बेचैनी बढ़ रही है तो नजदीकी डॉक्टर से तुरंत संपर्क करें।\n\nअगर आपके मन में खुद को नुकसान पहुंचाने के विचार आ रहे हों, तो तुरंत 108/102 पर कॉल करें या अपने पास के अस्पताल जाएं।"
          : "I hear you. If you're feeling low or stressed, do these 3 things now: 1) take 5 minutes of slow deep breathing, 2) drink water and talk to someone you trust, 3) contact a nearby doctor if distress is increasing.\n\nIf you have thoughts of harming yourself, call 108/102 immediately or go to the nearest hospital.";
      }
  ...
  const getSpeechLang = () => {
    ...
    return map[currentLanguage] || "en-IN";
  };
  ...
  const startVoiceSearch = () => {
    ...
    try {
      ...
      recognition.onstart = () => {
        setIsListening(true);
        toast.success("Listening... please speak your health query");
      };
      ...
      setIsListening(false);
      toast.error("Unable to start voice search.");
    }
  };
          ...
          <div className="p-4 border-t border-gray-200 bg-white rounded-b-xl relative z-10">
            <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-2">
              ...
              <Button
                ...
                size="icon"
                variant="outline"
                className="h-10 w-10 border-[#1B6CA8]/40 text-[#1B6CA8] hover:bg-[#1B6CA8]/10"
